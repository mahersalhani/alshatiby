...
import { useClassroomQuery } from '@/hooks/react-query/classroom';
import { useQueryState } from '@/hooks/use-query-state';

const ClassroomDataTable = () => {
  const t = useTranslations();
  const { query, setQuery } = useQueryState();
  const pageIndex = query?.pagination?.page - 1 || 0;
  const pageSize = query?.pagination?.pageSize || 20;

  const { data, isLoading } = useClassroomQuery(query);
  ...
  return (
    <div className="w-full">
      <div className="flex items-center py-4 px-5">
        <div className="flex-1 text-xl font-medium text-default-900">{t('classroom.list_title')}</div>
